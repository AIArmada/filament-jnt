---
title: Filament J&T Context
package: filament-jnt
status: current
surface: filament
family: checkout-flow
---

# Filament J&T Context

## Snapshot
- Composer: `aiarmada/filament-jnt`
- Role: Filament admin UI for J&T orders, tracking events, webhook logs, and sync actions.
- Search first: `src/Resources`, `src/Pages`, `src/Widgets`, `src/Actions`, `config`, `docs`
- Related: `jnt`, `shipping`, `filament-shipping`

## Read next
1. `docs/01-overview.md`
2. `docs/03-configuration.md`
3. `docs/04-usage.md`
4. `docs/99-troubleshooting.md`
5. `../jnt/CONTEXT.md` when gateway behavior or persistence changes are involved
6. `docs/02-installation.md` when plugin or panel setup changes are involved

## Guardrails
- Owns Filament resources, pages, widgets, tables, forms, and panel/plugin glue.
- Keep gateway rules, persistence, and webhook processing in `jnt`.
- Revalidate submitted IDs server-side; UI scoping is not authorization.
